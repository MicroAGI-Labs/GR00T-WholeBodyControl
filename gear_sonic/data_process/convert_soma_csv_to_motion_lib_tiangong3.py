...
# ruff: noqa: T201, DOC
"""Convert SOMA-retargeter CSV/PKL data to motion_lib format for TienKung 3 (tiangong3).

25-DOF sibling of ``convert_soma_csv_to_motion_lib_tiangong2dex.py`` (31-DOF) and
``convert_soma_csv_to_motion_lib.py`` (G1 29-DOF, untouched). Same structure / CLI /
output schema; only the robot-specific constants differ (DOF_AXIS, DOF/body counts,
IsaacLab<->MuJoCo reorder, joint/body name lists).

tiangong3 vs tiangong2dex: legs (12), waist (3) and head (2) are IDENTICAL in naming,
geometry and order. The ARMS differ: tiangong3 has 4 DOF per side --
shoulder_pitch/roll/yaw + elbow_pitch -- and NO elbow_yaw / wrist_pitch / wrist_roll.
The terminal arm link is ``elbow_pitch_{l,r}_link``. That drops 6 arm DOFs (3 per side),
25 DOF / 26 bodies total.

Produces the motion_lib PKL fields SONIC training expects: root_trans_offset, pose_aa,
dof, root_rot, smpl_joints, fps. See the tiangong2dex script's QUATERNION CONVENTION
VERDICT block -- the identical reasoning applies here (loader drives FK from pose_aa +
root_trans_offset only; stored root_rot is vestigial, written xyzw for parity).

Usage (mirrors the 2dex/G1 scripts):
    python gear_sonic/data_process/convert_soma_csv_to_motion_lib_tiangong3.py \
        --input  data/soma_retarget/<motion_or_dir> \
        --output data/tiangong3_test.pkl --fps 30 --fps_source 120

Self-test (no external retarget data needed):
    python gear_sonic/data_process/convert_soma_csv_to_motion_lib_tiangong3.py --selftest
"""
...
# ---------------------------------------------------------------------------
# Robot constants (TienKung 3 / tiangong3, 25 DOF / 26 bodies).
#
# Machine-derived from ``tiangong3.urdf`` and cross-checked in --selftest against
# the vendored mesh-free MJCF (mjcf/tiangong3.xml, produced by make_tiangong3_mjcf.py).
# Hardcoded (not imported) for the same "avoid heavy deps" reason as the 2dex/G1
# scripts: a plain-python conversion context has no isaaclab.
# ---------------------------------------------------------------------------

# MJ_TO_IL[mj] = il: for MuJoCo DOF slot mj, gives the IsaacLab DOF index (gather
# array; ``dof_mj = joint_pos_il[:, MJ_TO_IL]``). Only used by the IsaacLab-ordered
# input path (mode 1-3); the flat-CSV path (mode 4/5, what run_batch uses) tags the
# data joint_order='mj' and skips the reorder entirely. Re-derived from the
# IsaacLab-BFS body order vs the MuJoCo joint order in --selftest.
MJ_TO_IL = np.array(
    [0, 3, 6, 9, 14, 19, 1, 4, 7, 10, 15, 20, 2, 5, 8, 11, 16, 12, 17, 21, 23,
     13, 18, 22, 24],
    dtype=np.int32,
)

NUM_DOF = 25
NUM_BODIES = 26  # pelvis + 25 actuated links

# DOF_AXIS: rotation axis of each DOF in MuJoCo joint (document) order. Machine-derived
# from the URDF <axis> entries ordered by the MJCF hinge document order (URDF axes ==
# MJCF axes). Re-derived and asserted against the vendored MJCF in --selftest.
DOF_AXIS = np.array(
    [
        [0, 1, 0],  # 0  hip_pitch_l
        [1, 0, 0],  # 1  hip_roll_l
        ...
        [0, 1, 0],  # 21 shoulder_pitch_r
        [1, 0, 0],  # 22 shoulder_roll_r
        [0, 0, 1],  # 23 shoulder_yaw_r
        [0, 1, 0],  # 24 elbow_pitch_r
    ],
    dtype=np.float32,
)

# Joint names in MuJoCo (MJCF document) order -- the order the flat SOMA CSV DOF
# columns are written in. Consistent with the MJCF and the (future) robot module.
MUJOCO_JOINT_NAMES = [
    ...
    "shoulder_yaw_l_joint", "elbow_pitch_l_joint", "shoulder_pitch_r_joint",
    "shoulder_roll_r_joint", "shoulder_yaw_r_joint", "elbow_pitch_r_joint",
]

# Body names in MuJoCo (MJCF worldbody document) order (pelvis + 25 DOF links).
MUJOCO_BODY_NAMES = [
    ...
    "elbow_pitch_l_link", "shoulder_pitch_r_link", "shoulder_roll_r_link", "shoulder_yaw_r_link",
    "elbow_pitch_r_link",
]

# IsaacLab body order (pelvis + 25 links), from the breadth-first URDF traversal rule
# (children in URDF joint document order, fixed-joint children merged into parent) --
# the exact rule validated on H2/2dex. Used by --selftest to re-derive MJ_TO_IL.
...
# the SOMA-retargeter output; load_bones_csv reads DOF columns POSITIONALLY.
...
def load_bones_csv(csv_path: str) -> dict:
    ...
    in MuJoCo/document order.

    Rows are read POSITIONALLY (Frame + 3 trans + 3 rot + NUM_DOF dof) with the header
    SKIPPED (header=None, skiprows=1) -- never trusted -- exactly as the 2dex loader
    does, so it is robust to header naming/width. An exact field-count assertion
    (1 + 6 + NUM_DOF) guards against wrong-DOF CSVs (e.g. a 31-DOF 2dex CSV fed here).
    """
    import pandas as pd
    ...
    if ncols != expected:
        raise ValueError(
            ...
            f"target emitted a {NUM_DOF}-DOF CSV (Tiangong3_25DOF_CSVConfig); a 31-DOF "
            f"2dex CSV or a 29-DOF G1 CSV over {NUM_DOF}-DOF data will trip this."
        )

    root_pos = (arr[:, 1:4] / 100.0).astype(np.float32)  # cm -> m
    ...
    body_quat_w[:, :, 0] = 1.0  # identity wxyz for non-root bodies
...
def convert_sequence(seq_data: dict, fps: int, humanoid_fk=None) -> dict:  # noqa: ARG001
    """Convert a single sequence to motion_lib format (math identical to the 2dex/G1 scripts)."""
    joint_pos = seq_data["joint_pos"]  # (T, NUM_DOF)
    body_pos_w = seq_data["body_pos_w"]  # (T, NB, 3)
    body_quat_w = seq_data["body_quat_w"]  # (T, NB, 4) wxyz
    joint_order = seq_data.get("joint_order", "il")

    T = joint_pos.shape[0]

    root_trans_offset = body_pos_w[:, 0, :].copy()  # (T, 3)

    root_quat_wxyz = body_quat_w[:, 0, :]  # (T, 4) wxyz
    root_quat_xyzw = root_quat_wxyz[:, [1, 2, 3, 0]]  # (T, 4) xyzw
    ...
    return {
        ...
        "root_rot": root_quat_xyzw.astype(np.float32),  # xyzw; VESTIGIAL (loader ignores it)
...
def downsample_sequence(entry: dict, fps_source: int, fps_target: int) -> dict:
    """Stride-based downsample of a motion_lib entry (matches 2dex/G1 scripts)."""
...
def init_humanoid_fk():
    """Initialize Humanoid_Batch from the tiangong3 MJCF (parity with 2dex/G1 scripts).
...
def process_session_csvs(args_tuple):
    ...
    for csv_f in csv_files:
        ...
        try:
            ...
            # Surface the error -- do NOT silently swallow (the swallowed parallel-worker
            # traceback is what hid the 2dex ragged-CSV bug in the first pilot).
...
def run_selftest() -> int:
    ...
    print("[selftest] tiangong3 converter")
    ok = True
    ...
    check(NUM_DOF == 25 and NUM_BODIES == 26, f"NUM_DOF={NUM_DOF}, NUM_BODIES={NUM_BODIES}")
    ...
    check(sorted(MJ_TO_IL.tolist()) == list(range(NUM_DOF)), "MJ_TO_IL is a permutation of 0..24")
    ...
    if os.path.exists(mjcf):
        ...
        def walk(elem):
            for b in elem.findall("body"):
                ...
                for j in b.findall("joint"):
                    if j.get("type") in (None, "hinge"):  # saved MJCF hinges omit type=
        ...
        print(f"  [SKIP] MJCF not found at {mjcf} (run in the WBC clone to exercise this)")

    # --- try importing the robot module (only exists once an isaaclab config is added) ---
    try:
        from gear_sonic.envs.manager_env.robots.tiangong3 import (
            TIANGONG3_ISAACLAB_JOINTS,
            TIANGONG3_ISAACLAB_TO_MUJOCO_DOF,
        )

        check(list(TIANGONG3_ISAACLAB_JOINTS) == _ISAACLAB_BODY_ORDER,
              "robot module ISAACLAB_JOINTS == _ISAACLAB_BODY_ORDER")
        check(np.array_equal(np.array(TIANGONG3_ISAACLAB_TO_MUJOCO_DOF, dtype=np.int32), MJ_TO_IL),
              "robot module ISAACLAB_TO_MUJOCO_DOF == MJ_TO_IL")
    except Exception as e:  # noqa: BLE001
        print(f"  [SKIP] robot module import (no tiangong3 isaaclab config yet): {type(e).__name__}")
    ...
    qx = transform.Rotation.from_euler("xyz", [10, 20, 30], degrees=True).as_quat().astype(np.float32)
    q_wxyz = qx[[3, 0, 1, 2]]
    ...
    try:
        ...
        # --- WRONG DOF COUNT: a 31-field-DOF (2dex, 38 cols) CSV must raise here ---
        import csv as _csv
        ...
        with open(badf, "w", newline="") as fh:
            w = _csv.writer(fh)
            w.writerow(["Frame", *root_names, *[f"j{i}_dof" for i in range(31)]])  # 2dex width
            for t in range(Tn):
                w.writerow([t, 0.0, 0.0, 90.0, 0.0, 0.0, 0.0] + [0.0] * 31)  # 38 fields
        raised = False
        try:
            load_bones_csv(badf)
        except ValueError:
            raised = True
        os.unlink(badf)
        check(raised, "31-DOF (2dex-width) CSV raises ValueError against 25-DOF loader")
    except ImportError:
...
def main():
    parser = argparse.ArgumentParser(description="Convert SOMA CSV/PKL to motion_lib (TienKung 3, 25 DOF)")
    ...
    print(f"TienKung 3 {NUM_DOF} DOFs, {NUM_BODIES} bodies (hardcoded axes)")

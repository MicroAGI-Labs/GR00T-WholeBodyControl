...
class G1RobotDDS(DDSObject):
    ...
    def dds_publisher(self) -> Any:
        ...
        try:
            ...
            print(f"[g1_dds] dds_publisher called, data is None? {data is None}")
            if data is None:
                return
            ...
            if imu and len(imu) >= 13:
                ...
                imu_state.quaternion[:] = imu_array[[3, 4, 5, 6]] #[w,x,y,z] match real Unitree LowState (was [4,5,6,3]=xyzw)
            ...
            print(f"[g1_dds] published LowState tick={self.low_state.tick}")
            print(f"[g1_dds] published LowState tick={self.low_state.tick}")

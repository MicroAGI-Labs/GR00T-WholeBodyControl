        ...
        try:
            ...
            if now_ms - _obs_cache["dds_last_ms"] >= _obs_cache["dds_min_interval_ms"]:
                ...
                if g1_robot_dds:
                    ...
                    #       obs["floating_base_pose"][3:7]  (= pelvis qpos[3:7])
                    # while the torso goes to a SEPARATE rt/secondary_imu. Previously this
                    # published the TORSO IMU (imu_in_torso) here, so when the waist bent
                    # ~0.5 rad the torso pitched ~30 deg while the pelvis stayed upright and
                    # the controller saw a PHANTOM 30 deg tilt on its main balance IMU ->
                    # overcorrected -> toppled. Use the pelvis (use_torso_imu=False).

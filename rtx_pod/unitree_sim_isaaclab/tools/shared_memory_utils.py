...
def _detach_tracker(shm) -> None:
    """Opt an attached segment out of this process's resource_tracker so a
    reader never unlinks a segment it didn't create (CPython unlinks tracked
    segments on process exit even for opener-only). Without this, restarting
    the camera pub deletes the sim's live image segments -> cams=NONE.
    Best-effort. See SIM_RESILIENCE_PLAN.md Workstream C."""
    try:
        ...
        for image_name in image_names:
            try:
                ...
                if shm_name not in self.shms:
                    try:
                        self.shms[shm_name] = shared_memory.SharedMemory(name=shm_name)
                        _detach_tracker(self.shms[shm_name])  # attach-only: never unlink
                    except FileNotFoundError:
                ...
                # Drop the cached handle so the next call re-attaches — the segment
                # may have been replaced by a sim restart. WS-C.
                old = self.shms.pop(get_shm_name(image_name), None)
                if old is not None:
                    try:
                        old.close()
                    except Exception:

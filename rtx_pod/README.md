...
| `perturb_sim.py` | Atomically request a bounded, targeted world-frame pelvis force/torque from a running Isaac sim. Select one or more robot IDs or all robots; duration is in simulated seconds. |
...
| `sim_main.py` | (a) Hooks for `tools/stand_eval.py`: instantiate `StandEval` before the loop (`EVAL=1`), `arm()` on hold-release (cat-2/cat-3), `disarm()` on re-arm (cat-4), `update()` each control step. (b) Consumes atomic `$SIM_PERTURB_FILE` commands (default `/tmp/sim_perturbation.json`) and applies one-shot, per-environment world-frame pelvis wrenches for balance testing. (c) Multi-env reset/hold poses add Isaac's environment origins rather than stacking every robot at world zero, and the WebRTC viewport frames the complete grid. |
| `tools/sim_perturbation.py` | Dependency-free parser and safety validation for the perturbation command contract. |
